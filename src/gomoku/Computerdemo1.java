...
///////////////////
    ...
    public Square Mini_max1(int depth , char [][] u){
      ...
      public Square Mini_Max(int depth, char[][] u) {
        ...
        for (int i = 0; i < sixHighestScore.size(); i++) {
            ...
            if (max < currentScore) {
                max = currentScore;
                listApplicate.clear();
                listApplicate.add(sixHighestScore.get(i));
        ...
        return listApplicate.get(rand.nextInt(listApplicate.size()));//khí có nhiều nút cùng giá trị thì chọn ngẫu nhiên 1 nút
    }
      
    public int min_Score(int depth, char [][] square){  // person 
        int [][] mark  = new int [size][size] ;
      
        int Value = val(square);
       
        if (depth == 0 || Math.abs(Value) > 930) {
            ...
            for (int i = 0; i < sixHighestScore.size(); i++) {
                ...
                square[sixHighestScore.get(i).getRow()][sixHighestScore.get(i).getCol()] = 'x';
